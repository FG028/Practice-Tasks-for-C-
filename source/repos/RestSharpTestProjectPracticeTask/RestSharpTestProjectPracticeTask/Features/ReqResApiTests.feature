...
Scenario Outline: Update a user (PUT and PATCH)
    When I send a "<requestMethod>" request to "users/{userId}" with the following user data:
          | Field        | Value             |
          | name         | Updated Name      |
          | job          | Updated Job       |
    Then the status code should be 200
...
Scenario: Delete a user (negative case - non-existent user)
    When I send a DELETE request to "users/12345"
    Then the status code should be 404 (Not Found)

Scenario: Register a new user
    When I send a POST request to "auth/register" with the following user data:
          | Field        | Value               |
...
Scenario: Login a user
    When I send a POST request to "auth/login" with the following user data:
          | Field        | Value                |
...
Scenario: Get a delayed response
    When I send a GET request to "users/delayed"
    Then the status code should be 200
